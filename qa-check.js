...
function runQA(d, baseDir) {
  ...
  try {
    ...
    const shown = {};
    (html.match(/src="[^"]*\/images\/[^"]+"/g) || []).forEach(m => {
      const k = m.split('/').pop().replace(/"$/, '').replace(/-(?:800|1280)\.webp$/, '.webp');
      shown[k] = (shown[k] || 0) + 1;
    });
    Object.entries(shown).filter(([, n]) => n > 1).forEach(([k, n]) => {
      err('R18-repeat', `Одна картинка ${n} раза на странице: ${k} — секция выглядит как заглушка`);
    });
