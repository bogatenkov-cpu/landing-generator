...
function prepareTemplateData(data, langs) {
  ...
  floorplans = rawPlans.map(function(fp, i) {
    ...
    // arise-vibe's card has no dedicated price slot — without this the price
    // vanishes from the section entirely, so it goes in as the first feature.
    if (isAriseVibe && fpPrice) {
      details.unshift('<li class="' + liClass + '"><span>' + ui('price_from', langs) + '</span> <strong>' + t(fpPrice, langs) + '</strong></li>');
    }
    ...
    return {
      ...
      // angsana's card prints no price of its own — fall back to it so the
      // description line carries real information instead of rendering empty.
      desc: fp.description || fpPrice || '',
      price: fpPrice,
      ...
      // anchan-indigo names its panel .floorplans__plan and toggles
      // .floorplans__plan--active — a different suffix from annara's panel
      _active_class_plan: i === 0 ? ' floorplans__plan--active' : '',
  ...
  return {
    ...
    // Имя в шапке: короткая форма, если задана. Полное юридическое название
    // проекта не помещается в брендовый слот и режется многоточием.
    header_name: d.short_name || d.project_name || '',
...
function generateFromTemplate(data) {
  ...
  var globalFixCSS = '\n    img[src=""]{display:none}' +
    '\n    .header__logo{display:block;min-width:0;max-width:60vw;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}' +
    ...
    '\n    .header__inner,.header>.container,.header__container{gap:clamp(16px,2.5vw,40px)}' +
    '\n    .header__logo{flex:0 1 auto;min-width:0}' +
    // На узких экранах длинное название физически не влезает в одну строку:
    // переносим на две вместо многоточия — имя проекта важнее компактности
    '\n    @media (max-width:760px){.header__logo{white-space:normal!important;line-height:1.15;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;max-width:66vw}}' +
...
// Safety net for templates that forgot to guard an <img> with {{#field}}.
// A src-less <img> is hidden by CSS and by the runtime guard, but it still
// ships in the markup and costs a layout pass — drop it at build time so the
