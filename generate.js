...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    ${multiLang ? `span[data-lang]{display:none}body[data-lang="${defaultLang}"] [data-lang="${defaultLang}"]{display:inline}${langs.filter(l=>l!==defaultLang).map(l=>`body[data-lang="${l}"] [data-lang="${l}"]{display:inline}`).join('')}` : ''}
...
function prepareTemplateData(data, langs) {
  ...
  return {
    ...
    lang_css: multiLang ? 'span[data-lang]{display:none}' + langs.map(function(l) { return 'body[data-lang="' + l + '"] [data-lang="' + l + '"]{display:inline}'; }).join('') : '',
